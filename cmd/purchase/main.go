...
import (
	...
	purchase_service "github.com/jolinGalal/match/internal/purchase/service/purchase"
...
func main() {
	...
	var (
		...
		hostPort    = flag.String("host_port", "8095", "Server host port (valid values: 8090)")
	...
	{
		purchasesSvc = purchase_service.NewPurchases(logger, auth, dbgorm)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		purchasesEndpoints *purchases.Endpoints
	)
	{
		purchasesEndpoints = purchases.NewEndpoints(purchasesSvc)
	}
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, purchasesEndpoints, &wg, errc, logger, *dbgF)
		}
